...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const campaign = new Campaign({
      ...
      promotionSettings: category === 'promotion' ? promotionSettings : undefined,
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    if (reviewMinimumLength !== undefined) campaign.reviewMinimumLength = reviewMinimumLength;
    if (enableSmartFunnel !== undefined) campaign.enableSmartFunnel = enableSmartFunnel;
    if (promotionSettings) campaign.promotionSettings = { ...campaign.promotionSettings, ...promotionSettings };
    if (customization) campaign.customization = { ...campaign.customization, ...customization };
...
router.get('/stats/dashboard', auth, async (req, res) => {
  try {
    const totalCampaigns = await Campaign.countDocuments({ seller: req.user.id });
...
router.put('/:id/analytics', auth, async (req, res) => {
  try {
    ...
    if (completions !== undefined) campaign.analytics.totalCompletions += completions;
    if (redemptions !== undefined) campaign.analytics.totalRedemptions += redemptions;
